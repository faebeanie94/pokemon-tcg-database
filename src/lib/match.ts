...
export function matchCards(db: Database, request: MatchRequest): MatchResponse {
  ...
  if (request.set && request.name && request.number && !request.query) {
    ...
    if (line.parallel || line.notations.length || line.print_run) {
...
function matchSportsCards(db: Database, request: MatchRequest): MatchResponse {
  ...
  const setParsed = setRaw ? parseSportsSetName(setRaw) : null;
  const line = nameRaw
  ...
  const subject = request.subject?.trim() || line.subject_name;
  ...
  const interpretation: Interpretation = {
    name: line.display_name || nameRaw || undefined,
    number: numberNorm,
    language,
    game,
    parallel,
  ...
  for (const card of gatherSportsCandidates(db, {
    ...
    displayNorm: normalizeName(line.display_name || nameRaw),
    game,
    language,
  })) {
